...
async def start(message: Message, bot: Bot, state: State):
    edit_list.clear()
    try:
        await msg_photo.delete()
        await msg_2.delete()
    ...
    if message.text in ['/start', '/menu']:
        ...
        if ref[0:2] == '2_':
            ...
            await message.answer(text=main_text, reply_markup=markup, parse_mode='HTML', disable_web_page_preview=True)
    if info == None:
        await message.answer(text='Укажите город в котором будут публиковаться объявления')
        if ref == None:
            cur.execute(f"INSERT INTO users VALUES ('{message.chat.id}', '{message.chat.username}', '0', '0', 'None', 'None')")
        else:
            cur.execute(f"INSERT INTO users VALUES ('{message.chat.id}', '{message.chat.username}', '0', '0', '{ref}', 'None')")
    db.commit()
    db.close()
